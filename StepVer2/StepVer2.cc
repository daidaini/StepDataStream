...
namespace stepver2
{
    ...
    bool StepVer2::SetPackage(const std::string &src)
    {
    ...
    void StepVer2::ParseBaseRecord(const std::string &baseStr)
    {
    ...
    std::string StepVer2::BaseRecord() const
    {
        std::string result;
    ...
    std::string StepVer2::FormatedRecords(int start, int end)
    {
    ...
    void StepVer2::EndAppendRecord()
    {
        ...
        if (ptr == nullptr)
        {
            throw std::runtime_error("[StepVer2]MemoryPool size is not enough to store data..");
        }
    ...
    void StepVer2::GotoFirst()
    {
        currentRecIndex_ = 0;
    }

    void StepVer2::GotoNext()
    {
        ++currentRecIndex_;
    }

    std::pair<const char *, int> StepVer2::FindItem(int stepid)
    {
    ...
    std::pair<const char *, int> StepVer2::FindItemByBuffer(int stepid)
    {
        size_t pos = 0;
    ...
    std::string StepVer2::GetItem(int stepid)
    {
    ...
    std::string StepVer2::GetStepValueByID(int stepid)
    {
        std::pair<const char *, int> result = FindItem(stepid);
    ...
    std::string StepVer2::GetBaseFieldValue(int stepid)
    {
    ...
    bool StepVer2::SetFieldValue(int stepid, const char *value)
    {
        if (currentRecIndex_ < 0 || bodyRecords_.empty())
        {
        ...
        try
        {
            ...
            bodyRecords_[currentRecIndex_].length = std::min(int(currRec.size() - 1), blockSize_ - 1);
            return true;
        }
    ...
    bool StepVer2::SetFieldValueInt(int stepid, int value)
    {
        char cval[128]{};
    ...
    void StepVer2::SetBaseFieldValueInt(int stepid, int value)
    {
        baseRecord_[stepid] = std::to_string(value);
    }

    void StepVer2::SetBaseFieldValueString(int stepid, const std::string &value)
    {
        baseRecord_[stepid] = value; // 直接存储原始值，与ParseBaseRecord保持一致
    }

    std::string StepVer2::EscapeItem(const std::string &src)
    {
        std::string result;
    ...
    std::string StepVer2::EscapeBackItem(const std::string &src)
    {
